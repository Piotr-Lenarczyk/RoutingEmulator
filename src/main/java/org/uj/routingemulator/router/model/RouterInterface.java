...
import org.uj.routingemulator.router.exceptions.InterfaceAlreadyDisabledException;
import org.uj.routingemulator.router.exceptions.InterfaceAlreadyEnabledException;
...
public class RouterInterface implements NetworkInterface {
	 ...
	 * @throws InterfaceAlreadyDisabledException if the interface is already administratively disabled
	 */
	public void disable() {
		if (this.status.admin() == AdminState.ADMIN_DOWN) {
			logger.warning("Interface %s is already administratively disabled.".formatted(this.interfaceName));
			throw new InterfaceAlreadyDisabledException("Configuration path: [interfaces ethernet %s disable] already exists".formatted(this.interfaceName));
		}

		logger.finer("Disabling interface %s. Previous status: %s".formatted(this.interfaceName, this.status));
		this.status = new InterfaceStatus(AdminState.ADMIN_DOWN, this.status.link());
		logger.finest("Setting interface %s admin state to %s".formatted(this.interfaceName, this.status.admin()));
	}
	 ...
	 * @throws InterfaceAlreadyEnabledException if the interface is already administratively enabled
	 */
	public void enable() {
		if (this.status.admin() == AdminState.UP) {
			logger.warning("Interface %s is already enabled.".formatted(this.interfaceName));
			throw new InterfaceAlreadyEnabledException("Nothing to delete (the specified node does not exist)");
		}

		logger.finer("Enabling interface %s. Previous status: %s".formatted(this.interfaceName, this.status));
		this.status = new InterfaceStatus(AdminState.UP, this.status.link());
		logger.finest("Setting interface %s admin state to %s".formatted(this.interfaceName, this.status.admin()));
	}
	...
	public boolean isDisabled() {
		return this.status.admin() == AdminState.ADMIN_DOWN;
	}

	@Override
	public boolean isOperational() {
		return status != null && status.admin() == AdminState.UP;
	}
	...
	public void updateLinkState(NetworkTopology topology) {
		...
		this.status = new InterfaceStatus(this.status.admin(), newLinkState);
	}
}