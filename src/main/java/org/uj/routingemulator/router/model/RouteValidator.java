...
public class RouteValidator {
	...
	public static void validateSubnet(Subnet routeSubnet) {
		if (routeSubnet == null || !routeSubnet.isValidNetworkAddress()) {
			String subnetStr = routeSubnet == null ? "null" : routeSubnet.toString();
			logger.warning("Invalid subnet (not a network address) provided for route: " + routeSubnet);
			throw new InvalidSubnetException(subnetStr + " is not a valid IPv4 prefix", subnetStr);
		}
	}
	...
	public static void validateInterfaceAddress(InterfaceAddress interfaceAddress, String routerInterfaceName) {
		if (interfaceAddress.isNetworkAddress()) {
			...
			throw new InvalidAddressException(
					String.format("Cannot assign network address %s to the interface. Use a host address instead", interfaceAddress),
					InvalidAddressException.Reason.NETWORK_ADDRESS,
					interfaceAddress
			);
		}
		if (interfaceAddress.isBroadcastAddress()) {
			...
			throw new InvalidAddressException(
					String.format("Cannot assign broadcast address %s to the interface. Use a host address instead", interfaceAddress),
					InvalidAddressException.Reason.BROADCAST_ADDRESS,
					interfaceAddress
			);
		}
		if (!interfaceAddress.isValidHostAddress()) {
			...
			throw new InvalidAddressException(
					interfaceAddress + " is not a valid host IP address",
					InvalidAddressException.Reason.INVALID_HOST,
					interfaceAddress
			);
		}
	}
