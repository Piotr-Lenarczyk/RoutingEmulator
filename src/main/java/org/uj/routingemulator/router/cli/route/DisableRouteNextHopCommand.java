...
public class DisableRouteNextHopCommand implements RouterCommand {
	...
	public void execute(CommandExecutionContext context) {
		...
		try {
			...
			context.router().disableRoute(new StaticRoutingEntry(nextHopRouteParameters.dest(), nextHopRouteParameters.nh()));
			out.println("[edit]");
		} catch (RuntimeException e) {
			throw CLIErrorHandler.handleRouteException(e, CLIErrorHandler.formatDisableRouteNextHop(destinationSubnet, nextHop));
		}
	}
